...
const secretHasAlreadyBeenRevealed = localStorage.getItem(
  "secretHasAlreadyBeenRevealed"
);
...
on.load(() => {
  ...
  const updateImageSize = () => {
    ...
    state.view.fullyVisible =
      state.view.left === state.image.left &&
      state.view.right === state.image.right &&
      state.view.top === state.image.top &&
      state.view.bottom === state.image.bottom;
    ...
    state.region.right =
      1.0 + (state.view.right - state.image.right) / state.image.size;
    state.region.top = (state.view.top - state.image.top) / state.image.size;
    state.region.bottom =
      1.0 + (state.view.bottom - state.image.bottom) / state.image.size;
  ...
  const updateBrush = () => {
    ...
    if (
      state.colourTode.hand.state !== HAND.BRUSHING &&
      state.colourTode.hand.state !== HAND.PENCILLING
    )
      return;
    ...
    let [px, py] = getCursorView(
      state.cursor.previous.x,
      state.cursor.previous.y
    );
  ...
  const getSectionsOfWorldCell = (x, y) => {
    ...
    for (let wx = 0; wx < sectionSizeScale; wx++) {
      const gridX = Math.floor(
        (snappedX + (wx * WORLD_CELL_SIZE) / sectionSizeScale) * GRID_SIZE
      );
      for (let wy = 0; wy < sectionSizeScale; wy++) {
        const gridY = Math.floor(
          (snappedY + (wy * WORLD_CELL_SIZE) / sectionSizeScale) * GRID_SIZE
        );
        const sectionId = gridX * GRID_SIZE + gridY;
        const section = state.grid[sectionId];
  ...
  const updatePan = () => {
    ...
    if (
      ...
      hand.state === HAND.BRUSHING ||
      hand.state === HAND.PENCILLING
    ) {
      const cell = pickCell(...getCursorView(x, y));
      if (cell !== undefined)
        state.brush.hoverColour = Colour.splash(cell.colour);
    } else {
    ...
    if (!Mouse.Right) {
      if (dropperStartX !== undefined) {
        const dropperDistance = Math.hypot(
          x - dropperStartX,
          y - dropperStartY
        );
  ...
  const KEYDOWN = {};
  KEYDOWN.e = () =>
    (state.camera.mscaleTarget += state.camera.mscaleTargetControl);
  KEYDOWN.q = () =>
    (state.camera.mscaleTarget -= state.camera.mscaleTargetControl);
  ...
  const updateCamera = () => {
    if (state.camera.mscale !== state.camera.mscaleTarget) {
      ...
      const snap =
        state.camera.mscaleTarget *
        state.camera.mscaleTargetControl *
        state.camera.mscaleTargetSpeed;
      if (sign === 1 && state.camera.mscale > state.camera.mscaleTarget - snap)
        state.camera.mscale = state.camera.mscaleTarget;
      if (sign === -1 && state.camera.mscale < state.camera.mscaleTarget + snap)
        state.camera.mscale = state.camera.mscaleTarget;
    }
    ...
    if (state.camera.dx !== state.camera.dxTarget) {
      ...
      const sign = Math.sign(d);
      const snap =
        state.camera.dxTarget *
        state.camera.dsControl *
        state.camera.dsTargetSpeed;
      if (sign === 1 && state.camera.dx > state.camera.dxTarget - snap)
        state.camera.dx = state.camera.dxTarget;
      if (sign === -1 && state.camera.dx < state.camera.dxTarget + snap)
        state.camera.dx = state.camera.dxTarget;
    }
    ...
    if (state.camera.dy !== state.camera.dyTarget) {
      ...
      const sign = Math.sign(d);
      const snap =
        state.camera.dyTarget *
        state.camera.dsControl *
        state.camera.dsTargetSpeed;
      if (sign === 1 && state.camera.dy > state.camera.dyTarget - snap)
        state.camera.dy = state.camera.dyTarget;
      if (sign === -1 && state.camera.dy < state.camera.dyTarget + snap)
        state.camera.dy = state.camera.dyTarget;
    }
  ...
  const fireRandomSpotEvents = () => {
    let count = state.speed.dynamic
      ? state.speed.aer * state.cellCount
      : state.speed.count;
  ...
  const fireRandomSpotDrawEvents = () => {
    if (!state.view.visible) return;
    const count = state.speed.dynamic
      ? state.speed.aer * state.cellCount
      : state.speed.count;
  ...
  const aligns = (cells) => {
    ...
    const isAligned = tail.every(
      (cell) => cell.width === width && cell.height === height
    );
    return isAligned;
  };
  ...
  BEHAVE.set(Colour.Yellow.splash, (cell, redraw) => {
    ...
    const down = pickCell(
      cell.x + cell.width / 2,
      cell.y + cell.height / 2 + cell.height
    );
  ...
  BEHAVE.set(Colour.Cyan.splash, (cell, redraw) => {
    ...
    if (right !== undefined && aligns([cell, right])) {
      // Fall
      if (
        right.colour === Colour.Blue.splash ||
        right.colour === Colour.Cyan.splash
      ) {
      ...
      if (right.colour === Colour.Blue.splash) {
        ...
        if (
          head !== undefined &&
          head.colour === Colour.Black.splash &&
          head.width === cell.width * 2 &&
    ...
    if (left !== undefined && aligns([cell, left])) {
      // Fall
      if (
        left.colour === Colour.Blue.splash ||
        left.colour === Colour.Cyan.splash
      ) {
        const below = pickNeighbour(cell, 0, 1);
        if (below !== undefined && below.colour === Colour.Black.splash) {
  ...
  const DEBUG_RED_SPLIT = (cell, redraw) => {
    ...
    if (red === 0) {
      ...
      merged.colour = Math.max(
        11,
        Math.round((cell.colour + neighbour.colour) / 2)
      );
      //merged.colour = Math.max(11, Random.Uint8 % 100)
      drawCell(merged);
  ...
  const cloneDragonNumber = (number) => {
    ...
    const add =
      number.add === undefined ? undefined : cloneDragonNumber(number.add);
    const subtract =
      number.subtract === undefined
        ? undefined
        : cloneDragonNumber(number.subtract);
  ...
  const evaluateNumber = (number, args = {}) => {
    ...
    if (number.add !== undefined) {
      results = addChannelToResults(results, number.add, {
        source: args.source,
        multiplier: 1,
      });
    }
    if (number.subtract !== undefined) {
      results = addChannelToResults(results, number.subtract, {
        source: args.source,
        multiplier: -1,
      });
    }
  ...
  const getSplashesArrayFromArray = (array, args = {}) => {
    ...
    if (greens === undefined)
      greens = makeNumber({ channel: 1, variable: "green" });
    if (blues === undefined)
      blues = makeNumber({ channel: 2, variable: "blue" });
  ...
  makeRule = ({
    steps = [],
    transformations = DRAGON_TRANSFORMATIONS.NONE,
    locked = true,
    chance,
  } = {}) => {
  ...
  DRAGON_TRANSFORMATIONS = {
    NONE: [(x, y, w, h, W, H) => [x, y]],
    X: [(x, y, w, h, W, H) => [x, y], (x, y, w, h, W, fh) => [-x - w + W, y]],
    Y: [(x, y, w, h, W, H) => [x, y], (x, y, w, h, W, H) => [x, -y - h + H]],
    XY: [
      (x, y, w, h, W, H) => [x, y],
      (x, y, w, h, W, H) => [-x - w + W, y],
      (x, y, w, h, W, H) => [x, -y - h + H],
  ...
  const getTransformedRule = (rule, transformation, isTranslation) => {
    ...
    const transformedRule = makeRule({
      steps,
  ...
  const getTransformedDiagram = (
    ...
    for (let i = 0; i < left.length; i++) {
      ...
      const transformedLeftCell = getTransformedCell(
        leftCell,
        transformation,
        diagramWidth,
        diagramHeight,
    ...
    for (let i = 0; i < right.length; i++) {
      ...
      const transformedRightCell = getTransformedCell(
        rightCell,
        transformation,
        diagramWidth,
        diagramHeight,
    ...
    for (let i = 0; i < transformedRight.length; i++) {
      ...
      const additionalInstructionCount =
        diagramCell.splitX * diagramCell.splitY;
      const additionalInstructions = transformedRight.slice(
        i + 1,
        i + 1 + additionalInstructionCount
      );
      const orderedAdditionalInstructions = getOrderedCellAtoms(
        additionalInstructions
      );
      transformedRight.splice(
        i + 1,
        additionalInstructionCount,
        ...orderedAdditionalInstructions
      );
    }
  ...
  const getTransformedCell = (
    ...
    let [x, y, width, height] = transformation(
      cell.x,
      cell.y,
      cell.width,
      cell.height,
    ...
    if (!isTranslation) {
      const [testSplitX, testSplitY] = transformation(
        cell.splitX,
        cell.splitY,
        1,
        1,
      ...
      const [testWidth, testHeight] = transformation(
        cell.width,
        cell.height,
        1,
        1,
  ...
  const unregisterRegistry = ({ behaveFunctions }) => {
    state.dragon.behaves = state.dragon.behaves.filter(
      (behaveFunction) => !behaveFunctions.includes(behaveFunction)
    );
  };
  ...
  const makeBehaveFunction = (rule) => {
    ...
    for (const step of rule.steps) {
      ...
      const conditionFunction = makeConditionFunction(
        step,
        stampNames,
        rule.chance
      );
  ...
  const makeConditionFunction = (diagram, stampNames, chance) => {
    ...
    if (chance !== undefined) {
      ...
      const chanceValues = getNumbersFromDragonValues(chance.values)
        .map((v) => 2 ** (9 - v))
        .map((v) => (v === 512 ? 0 : v));
      if (chanceValues.length === 1) {
        ...
        chanceCondition = () =>
          oneIn(chanceValues[Random.Uint8 % chanceValues.length]);
      }
    }

    const conditionFunction = (origin) => {
      if (chance !== undefined && !chanceCondition())
  ...
  const makeResultFunction = (diagram, stampNames) => {
    ...
    return (neighbours, redraw, stamps) => {
      ...
      for (const instruction of results) {
        ...
        const {
          drawn: resultDrawn,
          bonusTargets: resultBonusTargets,
          skip: resultSkip,
        } = result;
    ...
    const instruction = (target, redraw, neighbours, neighbourId, stamps) => {
      ...
      if (isDynamic) {
        ...
        for (let i = 0; i < cell.content.channels.length; i++) {
          ...
          let results = VARIABLE_EVALUATOR[channel.variable](channel, {
            source,
          });
          if (channel.add !== undefined) {
            results = addChannelToResults(results, channel.add, {
              source,
              multiplier: 1,
            });
          }
          if (channel.subtract !== undefined) {
            results = addChannelToResults(results, channel.subtract, {
              source,
              multiplier: -1,
            });
          }
          const choices = results
            .map((v, i) => (v === true ? i : false))
            .filter((v) => v !== false);
  ...
  const addChannelToResults = (
    ...
    const addendChoices = addendResults
      .map((v, i) => (v === true ? i : false))
      .filter((v) => v !== false);
    const augendChoices = augendResults
      .map((v, i) => (v === true ? i : false))
      .filter((v) => v !== false);
    ...
    if (addend.add !== undefined) {
      results = addChannelToResults(results, addend.add, {
        source,
        multiplier: 1,
      });
    }
  ...
  debugDiagramCell = (cell, { read = false } = {}) => {
    if (read) {
      print(
        ...
        cell.height,
        "for",
        getSplashesArrayFromArray(cell.content)
      );
    } else {
      if (cell.instruction.type === "NOTHING") {
        print(
          cell.instruction.type,
          "at",
          cell.x,
          cell.y,
          "with size",
  ...
  const WATER_RIGHT_SLIP = makeDiagram({
    left: [makeDiagramCell({ x: 0, y: 0, width: 1, content: BLUE })],
    right: [
      makeDiagramCell({
        x: 0,
        y: 0,
  ...
  const WATER_RIGHT_SPAWN_DIAGRAM = makeDiagram({
    left: [makeDiagramCell({ x: 0, y: 0, content: PURPLE })],
    right: [
      makeDiagramCell({
        x: 0,
        y: 0,
...
registerRule(
  ...
  RAINBOW_DIAGRAM = makeDiagram({
    left: [makeDiagramCell({ content: RAINBOW })],
  });
  ...
  RAINBOW_DIAGRAM_2 = makeDiagram({
    left: [makeDiagramCell({ content: RAINBOW2 })],
  });

  const WATER_SPAWN_DIAGRAM_CYAN = makeDiagram({
    left: [makeDiagramCell({ x: 0, y: 0, content: PURPLE })],
    right: [makeDiagramCell({ x: 0, y: 0, content: CYAN })],
  });

  const WATER_SPAWN_DIAGRAM_BLUE = makeDiagram({
    left: [makeDiagramCell({ x: 0, y: 0, content: PURPLE })],
    right: [makeDiagramCell({ x: 0, y: 0, content: BLUE })],
  });
  ...
  const WATER_SWAP_BLUE = makeDiagram({
    left: [makeDiagramCell({ x: 0, y: 0, content: BLUE })],
    right: [makeDiagramCell({ x: 0, y: 0, content: CYAN })],
  });

  const WATER_SWAP_CYAN = makeDiagram({
    left: [makeDiagramCell({ x: 0, y: 0, content: CYAN })],
    right: [makeDiagramCell({ x: 0, y: 0, content: BLUE })],
  });
  ...
  const updateHand = () => {
    ...
    if (
      ...
      Mouse.position[0] !== undefined &&
      hand.previous.x !== undefined
    ) {
      ...
      const sum = hand.velocityHistory.reduce(
        (a, b) => ({ x: a.x + b.x, y: a.y + b.y }),
        { x: 0, y: 0 }
      );
      const average = {
        x: sum.x / hand.velocityHistory.length,
        y: sum.y / hand.velocityHistory.length,
      };
  ...
  const colourTodeDraw = () => {
    colourTodeContext.clearRect(
      0,
      0,
      colourTodeCanvas.width,
      colourTodeCanvas.height
    );
    /*if (!NO_FOOLS_MODE) {
  ...
  HAND.FREE = {
    ...
    mousemove: (e) => {
      ...
      if (atom !== undefined) {
        if (atom.grabbable) {
          if (!Mouse.Left) {
            if (atom.cursor !== undefined)
              changeHandState(HAND.HOVER, atom.cursor(atom, HAND.HOVER));
  ...
  HAND.TOUCHING = {
    ...
    mousemove: (e) => {
      ...
      const distanceFromPityStart = Math.hypot(
        e.clientX - hand.pityStartX,
        e.clientY - hand.pityStartY
      );
      ...
      if (!hand.content.dragLockX)
        hand.content.x =
          (hand.pityStartX +
            dampen(dx, hand.content.attached && !hand.content.noDampen)) /
            CT_SCALE +
          hand.offset.x;
      if (!hand.content.dragLockY)
        hand.content.y =
          (hand.pityStartY +
            dampen(dy, hand.content.attached && !hand.content.noDampen)) /
            CT_SCALE +
          hand.offset.y;

      hand.content.x = clamp(
        hand.content.x,
        hand.content.minX,
        hand.content.maxX
      );
      hand.content.y = clamp(
        hand.content.y,
        hand.content.minY,
        hand.content.maxY
      );
      ...
      if (hand.content.draggable) {
        ...
        const attached =
          hand.content.attached &&
          !hand.content.dragOnly &&
          !hand.content.noDampen;

        hand.content = hand.content.drag(hand.content, x, y);

        if (!hand.content.dragLockX)
          hand.content.x =
            (hand.pityStartX + dampen(dx, attached)) / CT_SCALE + hand.offset.x;
        if (!hand.content.dragLockY)
          hand.content.y =
            (hand.pityStartY + dampen(dy, attached)) / CT_SCALE + hand.offset.y;
  ...
  HAND.DRAGGING = {
    ...
    mousemove: (e) => {
      if (!hand.hasStartedDragging) {
        ...
        hand.content = hand.content.drag(
          hand.content,
          e.clientX / CT_SCALE,
          e.clientY / CT_SCALE
        );
      ...
      if (!hand.content.dragLockX)
        hand.content.x = e.clientX / CT_SCALE + hand.offset.x;
      if (!hand.content.dragLockY)
        hand.content.y = e.clientY / CT_SCALE + hand.offset.y;
    ...
    mouseup: (e) => {
      ...
      if (
        hand.content.highlighter &&
        hand.content.highlightedAtom !== undefined
      ) {
  ...
  on.mousemove((e) =>
    hand.state.mousemove ? hand.state.mousemove(e) : undefined
  );
  on.mousedown((e) => {
    if (e.button === 0) if (hand.state.mousedown) hand.state.mousedown(e);
    if (e.button === 1)
      if (hand.state.middlemousedown) hand.state.middlemousedown(e);
  });
  on.mouseup((e) => {
    if (e.button === 0) if (hand.state.mouseup) hand.state.mouseup(e);
    if (e.button === 1)
      if (hand.state.middlemouseup) hand.state.middlemouseup(e);
  });
  ...
  const deleteChild = (parent, child, { quiet = false } = {}) => {
    ...
    if (id === -1) {
      if (quiet) return;
        ...
        throw new Error(`Can't delete child of atom because I can't find it!`);
    }
  ...
  const giveChild = (parent, atom) => {
    ...
    deleteAtom(atom);
    if (atom.stayAtBack || atom.behindOtherChildren)
      parent.children.unshift(atom);
  ...
  const getColourCycleLength = (atom) => {
    let length = Math.max(
      COLOUR_CYCLE_LENGTH / atom.colours.length,
      COLOUR_CYCLE_SPEED
    );
      ...
      if (atom.hasBorder) {
        if (atom.hasInner) {
          ...
          if (atom.borderColour === undefined) {
            ...
            if (atom.isTool) {
              colourTodeContext.fillStyle =
                toolBorderColours[atom.colour.splash];
              border *= 1.5;
              /*W += BORDER_THICKNESS
							H += BORDER_THICKNESS
							Y -= BORDER_THICKNESS/2*/
            } else if (
              atom.width === atom.height /* || atom.isTallRectangle*/
            ) {
              border *= 1.5;
            }
          } else {
          ...
          if (atom.stamp !== undefined) {
            colourTodeContext.arc(
              X + R,
              Y + R,
              Math.round((PADDLE_HANDLE.size - OPTION_MARGIN / 2) / 2),
              0,
              2 * Math.PI
            );
          }
          ...
          if (atom.stamp !== undefined) {
            colourTodeContext.arc(
              X + R,
              Y + R,
              Math.round((PADDLE_HANDLE.size - OPTION_MARGIN / 2) / 2) + border,
              0,
              2 * Math.PI
            );
          }
  ...
  const COLOURTODE_SQUARE = {
    ...
    click: (atom) => {
      if (atom.joins.length > 0) {
        if (
          atom.parent === COLOURTODE_BASE_PARENT ||
          !atom.parent.pinhole.locked
        ) {
    ...
    createPicker: (atom) => {
      ...
      if (atom.value.channels[2] !== undefined) {
        if (atom.value.channels[2].variable === undefined) {
          ...
          blue.x +=
            COLOURTODE_PICKER_PAD_MARGIN +
            3 * (COLOURTODE_SQUARE.size + COLOURTODE_PICKER_PAD_MARGIN);
          ...
          diamond.x =
            (COLOURTODE_PICKER_PAD_MARGIN + COLOURTODE_SQUARE.size) * 3 +
            (COLOURTODE_SQUARE.size + COLOURTODE_PICKER_PAD_MARGIN) / 2 -
            diamond.width / 3;
      ...
      if (atom.value.channels[1] !== undefined) {
        if (atom.value.channels[1].variable === undefined) {
          ...
          green.x +=
            COLOURTODE_PICKER_PAD_MARGIN +
            2 * (COLOURTODE_SQUARE.size + COLOURTODE_PICKER_PAD_MARGIN);
          ...
          diamond.updateAppearance(diamond);
          diamond.x =
            (COLOURTODE_PICKER_PAD_MARGIN + COLOURTODE_SQUARE.size) * 2 +
            (COLOURTODE_SQUARE.size + COLOURTODE_PICKER_PAD_MARGIN) / 2 -
            diamond.width / 3;
          diamond.y = atom.height / 2 - diamond.height / 2;
          diamond.attached = true;
        ...
        if (atom.highlightedAtom === undefined)
          for (const paddle of paddles) {
            ...
            if (paddle.cellAtoms.length === 0) {
              ...
              left > pleft + paddle.rightTriangle.x
            ) {
              ...
              for (const cellAtom of paddle.cellAtoms) {
                ...
                if (
                  !isCellAtomSpotFilled(paddle, spotLeft) &&
                  dspotLeft < winningDistance
                ) {
                ...
                const dspotAbove = Math.hypot(
                  x - spotAbove[0],
                  y - spotAbove[1]
                );
                if (
                  !isCellAtomSpotFilled(paddle, spotAbove) &&
                  dspotAbove < winningDistance
                ) {
                ...
                const dspotRight = Math.hypot(
                  x - spotRight[0],
                  y - spotRight[1]
                );
                if (
                  !isCellAtomSpotFilled(paddle, spotRight) &&
                ...
                const dspotBelow = Math.hypot(
                  x - spotBelow[0],
                  y - spotBelow[1]
                );
                if (
                  !isCellAtomSpotFilled(paddle, spotBelow) &&
                  dspotBelow < winningDistance
                ) {
              ...
              if (winningSide === "left") {
                ...
                atom.highlight.x =
                  cx - HIGHLIGHT_THICKNESS / 2 + winningCellAtom.width;
                atom.highlight.y = cy;
                ...
                atom.highlight.x = cx;
                atom.highlight.y =
                  cy - HIGHLIGHT_THICKNESS / 2 + winningCellAtom.height;
              }
    ...
    drop: (atom) => {
      if (atom.highlight !== undefined) {
        if (atom.highlightedAtom.isPaddle) {
          ...
          atom.highlightedAtom.isSquare &&
          atom.highlightedAtom.parent.isPaddle &&
          !(
            atom.highlightedAtom.joins.length > 0 &&
            atom.highlightedAtom.joinExpanded
          )
        ) {
    ...
    joinExpand: (atom) => {
      ...
      pickerPad.height =
        (atom.joins.length + 1) * (atom.height + OPTION_MARGIN) + OPTION_MARGIN;
  ...
  const COLOURTODE_PICKER_CHANNEL = {
    ...
    drag: (atom) => {
      if (atom.parent.isSquare) {
        ...
        const operationName =
          atom.highlightedSlot === "padTop" ? "add" : "subtract";
        ...
        if (diamond.expanded) {
          ...
          const handle =
            atom.highlightedSlot === "padTop" ? "handleTop" : "handleBottom";
    ...
    construct: (atom) => {
      ...
      const selectionBottom = createChild(
        atom,
        COLOURTODE_CHANNEL_SELECTION_END
      );
    ...
    positionSelection: (atom, start, end, top, bottom) => {
      if (!atom.expanded) {
        ...
        atom.selectionBottom.y =
          start + optionSpacing - atom.selectionBottom.height;
        ...
        atom.selectionBottom.maxY =
          bottom - atom.selectionBottom.height + optionSpacing;
      }
    ...
    positionSelectionBack: (atom) => {
      ...
      atom.selectionBack.height =
        atom.selectionBottom.y - atom.selectionTop.y + atom.selectionTop.height;
      atom.selectionBack.width =
        atom.width + COLOURTODE_CHANNEL_SELECTION_END.height * 2;
    },
      ...
      if (hand.content === atom && hand.state === HAND.DRAGGING) {
        ...
        for (const square of atoms) {
          ...
          if (other.isTallRectangle) {
            ...
            for (const slotName of slotNames) {
              ...
              while (
                endAtom.isTallRectangle &&
                endAtom.operationAtoms[slotName] !== undefined
              ) {
            ...
            const slots = ["red", "green", "blue"].filter(
              (slot) => square[slot] === undefined
            );
            ...
            for (const slot of slots) {
              ...
              const sx =
                ...
                OPTION_MARGIN * 2 +
                slotId *
                  (COLOURTODE_SQUARE.size + COLOURTODE_PICKER_PAD_MARGIN);
        ...
        if (winningSquare !== undefined) {
          ...
          atom.highlight.x =
            ...
            winningSquare.size +
            OPTION_MARGIN +
            slotId * (OPTION_MARGIN + winningSquare.size);
          atom.highlight.y = ay;
          ...
          for (const paddle of paddles) {
            ...
            if (
              ...
              left <= pright &&
              right >= pright &&
              ((top < pbottom && top > ptop) ||
                (bottom > ptop && bottom < pbottom))
            ) {
    ...
    drop: (atom) => {
      if (atom.highlight !== undefined) {
        if (atom.highlightedAtom.isSquare) {
          ...
          square.receiveNumber(square, atom.value, slotId, {
          ...
          atom.y =
            atom.highlightedAtom.y +
            atom.highlightedAtom.height / 2 -
            atom.height / 2;
          atom.dx = 0;
          atom.dy = 0;
    ...
    updateColours: (atom) => {
      ...
      if (atom.parent.isSquare) {
        ...
        if (redNumber === undefined)
          redNumber = makeNumber({
            channel: 0,
            values: [
              true,
              false,
        ...
        if (greenNumber === undefined)
          greenNumber = makeNumber({
            channel: 1,
            values: [
              true,
              false,
        ...
        if (blueNumber === undefined)
          blueNumber = makeNumber({
            channel: 2,
            values: [
              true,
              false,
        ...
        parentR = makeNumber({
          values: [...redNumber.values],
          channel: redNumber.channel,
        });
        parentG = makeNumber({
          values: [...greenNumber.values],
          channel: greenNumber.channel,
        });
        parentB = makeNumber({
          values: [...blueNumber.values],
          channel: blueNumber.channel,
        });
      } else {
        const values = [
          false,
      ...
      if (mainParentChannel !== undefined)
        mainParentChannel.values = [
          false,
          false,
          false,
          false,
    ...
    createOptions: (atom) => {
      ...
      if (startId === undefined)
        throw new Error(
          "[ColourTode] Number cannot be NOTHING. Please let @TodePond know if you see this error!"
        );
      ...
      for (let i = 0; i < 10; i++) {
        ...
        const option = createChild(atom, {
          ...COLOURTODE_PICKER_CHANNEL_OPTION,
          pityTop,
          pityBottom,
        });
  ...
  const COLOURTODE_CHANNEL_SELECTION_END = {
    ...
    drop: (atom) => {
      let distanceFromMiddle = Math.round(
        (atom.y + CHANNEL_HEIGHT / 2) / OPTION_SPACING
      );
  ...
  const COLOURTODE_TALL_RECTANGLE = {
      ...
      for (const other of atoms) {
        ...
        for (const slot of slots) {
          ...
          const sx =
            ...
            other.size +
            OPTION_MARGIN * 2 +
            slotId * (COLOURTODE_SQUARE.size + COLOURTODE_PICKER_PAD_MARGIN);
          const sy = ay + OPTION_MARGIN;
          const distance = Math.hypot(x - sx, y - sy);
          if (distance < winningDistance) {
            winningDistance = distance;
    ...
    place: (atom, highlightedAtom) => {
      ...
      if (!highlightedAtom.isSquare) {
        ...
        atom.x = 0;
        atom.y =
          highlightedAtom.y + highlightedAtom.height / 2 - atom.height / 2;
        giveChild(diamond, atom);
        //diamond.expand(diamond)
      ...
      if (atom.value.add !== undefined) {
        if (atom.operationAtoms.padtop === undefined) {
          if (atom.value.add.variable === undefined) {
            ...
            operationAtom.y =
              atom.padTop.y + atom.padTop.height / 2 - operationAtom.height / 2;
      ...
      if (atom.value.subtract !== undefined) {
        if (atom.operationAtoms.padBottom === undefined) {
          if (atom.value.subtract.variable === undefined) {
            ...
            operationAtom.y =
              atom.padBottom.y +
              atom.padBottom.height / 2 -
              operationAtom.height / 2;
    ...
    expand: (atom) => {
      ...
      if (atom.value.add === undefined) {
        if (
          atom.y < 0 ||
          !(
            atom.parent.isTallRectangle &&
            atom.parent.operationAtoms.padBottom === atom
          )
        ) {
      ...
      if (atom.value.subtract === undefined) {
        if (
          ...
          !(
            atom.parent.isTallRectangle &&
            atom.parent.operationAtoms.padTop === atom
          )
        ) {
      ...
      atom.padRight.width =
        OPTION_MARGIN + (atom.width + OPTION_MARGIN / 1.5) * 3;
      atom.padRight.y = atom.height / 2 - atom.padRight.height / 2;
      atom.padRight.x =
        atom.width / 2 +
        (COLOURTODE_SQUARE.size + COLOURTODE_PICKER_PAD_MARGIN * 2) / 2 +
        OPTION_MARGIN;
      ...
      atom.green.x =
        atom.padRight.x +
        OPTION_MARGIN / Math.SQRT2 +
        (atom.green.width + OPTION_MARGIN) * 1;
      ...
      atom.blue.x =
        atom.padRight.x +
        OPTION_MARGIN / Math.SQRT2 +
        (atom.blue.width + OPTION_MARGIN) * 2;
  ...
  const DIAMOND_CHOICE = {
    ...
    click: (atom) => {
      ...
      top.receiveNumber(top, topChannel.value, channelNumber, {
        expanded: topChannel.expanded,
        numberAtom: topChannel,
      });
    },
  };
  ...
  const PADDLE = {
    ...
    drag: (paddle, x, y) => {
      if (paddle.pinhole.locked) {
        ...
        registerAtom(square);
        state.brush.colour = makeDiagram({
          left: [makeDiagramCell({ content: diagram })],
        });
  ...
  const updatePaddleSize = (paddle) => {
    ...
    if (paddle.rightTriangle !== undefined) {
      for (const cellAtom of paddle.cellAtoms) {
        ...
        slot.x =
          cellAtom.x + paddle.rightTriangle.x + paddle.rightTriangle.width;
        slot.y = cellAtom.y;
    ...
    if (paddle.symmetryCircle !== undefined) {
      ...
      paddle.symmetryCircle.y =
        paddle.height / 2 - paddle.symmetryCircle.height / 2;
    }
  ...
  const updatePaddleRule = (paddle) => {
    ...
    for (const cellAtom of orderedCellAtoms) {
      ...
      const rightContent =
        cellAtom.slotted === undefined ? undefined : cellAtom.slotted.value;
    ...
    const chance =
      paddle.chance === undefined ? undefined : paddle.chance.value;
    const rule = makeRule({
      steps: [diagram],
      transformations,
      locked,
  ...
  const CIRCLE = {
    draw: (atom) => {
      ...
      if (atom.hasBorder) {
        ...
        colourTodeContext.fillStyle =
          atom.borderColour !== undefined ? atom.borderColour : Colour.Void;
        colourTodeContext.beginPath();
        colourTodeContext.arc(X, Y, R, 0, 2 * Math.PI);
        colourTodeContext.fill();
        let borderScale =
          atom.borderScale !== undefined ? atom.borderScale : 1.0;
  ...
  const TRIANGLE_PICK_UP = {
    ...
    x:
      TRIANGLE_PAD.x +
      TRIANGLE_PAD.width / 2 -
      (COLOURTODE_SQUARE.size - OPTION_MARGIN * 1.5) / 2,
  ...
  const TRIANGLE_PICK_RIGHT = {
    ...
    x:
      TRIANGLE_PAD.x +
      TRIANGLE_PAD.width / 2 -
      ((COLOURTODE_SQUARE.size - OPTION_MARGIN * 1.5) * Math.sqrt(3)) / 2 / 2,
  ...
  const TRIANGLE_PICK_DOWN = {
    ...
    y:
      TRIANGLE_PAD.y +
      TRIANGLE_PAD.height -
      (COLOURTODE_SQUARE.size - OPTION_MARGIN * 1.5) -
      OPTION_MARGIN / 2,
  };
  ...
  const SYMMETRY_TOGGLE_X = {
    ...
    x:
      SYMMETRY_PAD.x +
      SYMMETRY_PAD.width / 2 -
      (COLOURTODE_SQUARE.size - OPTION_MARGIN) / 2,
  ...
  const SYMMETRY_TOGGLE_R = {
    ...
    y:
      SYMMETRY_PAD.y +
      SYMMETRY_PAD.height -
      (COLOURTODE_SQUARE.size - OPTION_MARGIN) -
      OPTION_MARGIN / 2,
  };
  ...
  const wideRectangleTool = addMenuTool(
    COLOURTODE_PICKER_CHANNEL,
    "wide_rectangle"
  );
  //menuRight += BORDER_THICKNESS
  const tallRectangleTool = addMenuTool(
    COLOURTODE_TALL_RECTANGLE,
    "tall_rectangle"
  );
  createPaddle();
